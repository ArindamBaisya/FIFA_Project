...
library(dplyr)
FIFA_Data<-read.csv(file.choose())
newdata = FIFA_Data[order(-FIFA_Data$overall),]
z=head(newdata,10)
z[,c(2,4,15,17,20)]
...
```
```{r}
x=data.frame(FIFA_Data$def, FIFA_Data$sho)
km.out <- kmeans(head(x,1000), centers = 3)
plot(head(x,1000), 
  col = km.out$cluster,
...
#SUpport Vector Machines gives the best model
...
```{r}
#Linear Regression to identify the most valuable players
...
summary(vall)
predd <- predict(vall, data = testsd)
meanlr=mean(sqrt((predd - testsd$eur_value)^2))
meanlr
```
...
meanlrm=mean(sqrt((predd2 - testsd$eur_value)^2))
meanlrm
#Linear Regression produced an error of 3616779
```
...
pred3 <- predict(fit.ridge, s = bestlam.ridge, newx = test.mat) 
length(pred3)
meanridge=mean((pred3 - testsd$eur_value)^2)  
coef(cv.ridge)
...
bestlam.lasso
pred4 <- predict(fit.lasso, s = bestlam.lasso, newx = test.mat) 
meanlasso=mean(sqrt((pred4 - testsd$eur_value)^2))
coef(fit.lasso)
...
meanpolyL=mean(sqrt((predd - testsd$eur_value)^2))
...
meanpolyFSS=mean(sqrt((predd - testsd$eur_value)^2))
# As per forward stepwise age, eur_wage, eur_release_cause, overall, potential, dri, def, international reputation, skill moves, weak foot, heading accuracy and reactions are the significant predictors
```
...
summary(vall)
predd <- predict(vall, data = testsd)
meansvmLR=mean(sqrt((predd - testsd$eur_value)^2))
...
predd <- predict(vall4, data = testsd)
meansvmFSS=mean(sqrt((predd - testsd$eur_value)^2))
#Support Vector Regression gives the least error and hence is a method to use.
```

```{r}
data.frame("Methods Used"=c("Ridge Regression","Lasso","Polynomial Regresson Using LR Best Parameters","Polynomial Reg. using Best Subset Selection Parameters","SVM using LR Best Parameters","SVM using Best Subset Selection"),"Values"=c(meanridge,meanlasso,meanpolyL,meanpolyFSS,meansvmLR,meansvmFSS))
```
...
ggplot(data = FIFA_Data, aes(x=overall, y = eur_value))+
  ...
  geom_smooth(method = 'gam', formula = y~s(x,bs = "cs"))+ggtitle("Value and Overall ")+
...
ggplot(data = FIFA_Data, aes(x=potential, y = eur_value))+
  ...
  geom_smooth(method = 'gam', formula = y~s(x,bs = "cs"))+ggtitle("Value and Potential ")+
...
ggplot(data = FIFA_Data, aes(x=international_reputation, y = eur_value))+
  ...
  geom_smooth(method = 'lm')+ggtitle("Value and Reputation ")+
...
ggplot(data = FIFA_Data, aes(eur_release_clause, y = eur_value))+
  ...
  geom_smooth(method = 'lm')+ggtitle("Value and Release Clause ")+
...
dfbb
ggplot(data = cjs, aes(x=cjs$name, y=cjs$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('Valuable Players')+ylab('Value')+ggtitle("Most Valuable Players")
```
...
ggplot(data = goalies, aes(x=goalies$name, y=goalies$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab("Goalkeepers")+ylab('Value')+ggtitle("Most Valuable Goalkeepers")
goalies$name[1]
VA <- c(" ________________________________________________________________ ")  
VB <- c("|           |      |____Home Goalpost____|       |               |")
VC <- c("|           |                DeGEA               |               |")
...
dfbb2
ggplot(data = cjs2, aes(x=cjs2$name, y=cjs2$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('Defenders')+ylab('Value')+ggtitle("Most Valuable Defender")
```
...
dfbb3
ggplot(data = cjs3, aes(x=cjs3$name, y=cjs3$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('LB')+ylab('Value')+ggtitle("Most Valuable Left Back")
```
...
dfbb3
ggplot(data = cjs4, aes(x=cjs4$name, y=cjs4$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('RB')+ylab("Value")+ggtitle("Most Valuable Right Back")
...
dfbb3
ggplot(data = cjs5, aes(x=cjs5$name, y=cjs5$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('Midfielders')+ylab('Value')+ggtitle("Most Valuable Midfielders")
```
...
ggplot(data = cjs6, aes(x=cjs6$name, y=cjs6$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('Def Midfielders')+ylab('Value')+ggtitle("Most Valuable Defensive Midfielders")
...
dfbb5
ggplot(data = cjs7, aes(x=cjs7$name, y=cjs7$eur_value))+geom_bar(stat="identity", aes(fill=club))+xlab('Strikers')+ylab('Value')+ggtitle("Most Valuable Striker")
